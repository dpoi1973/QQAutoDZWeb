class fakegoodsnamecomponentComponentController {
  // @ngInject
  constructor(CommonUtilsService, $log, $window, $scope, $timeout) {
    ...
    //     if (structlist[i]) {
    //       this.$log.log(i);
    //       this.$log.log(structlist[i]);
      model.content = structlist;
    //     } else {
    //       model.content = '';
    //     }
      ...
      if(keycode!=38&&keycode!=40&&keycode!=13){
        ...
        if(keycode==13){
            ...
            document.getElementById('auto').style.display = 'none';
               ...
               // range.setStart(el, this.curcursor);
                range.collapse(true);
                sel.removeAllRanges();
                sel.addRange(range);
                el.focus();
  ...
  ok() {
    ...
    //     }
    //   }, this)
   this.$log.log(document.getElementById("changevalue").innerHTML);

      const finresult = {};
      finresult.hode = document.getElementById("changevalue").innerHTML
...
export const fakegoodsnamecomponent = {
  template: require('./fakegoodsnamecomponent.html'),
  bindings: {
    resolve: '<',
    close: '&',
    dismiss: '&'
  },
  controller: fakegoodsnamecomponentComponentController
};