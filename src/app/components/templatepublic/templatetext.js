...
class templatetextComponentController extends baseGridComponent {
  // @ngInject
  ...
  okbutton() {
    ...
    for (let i = 0; i < this.SelectedRows.length; i++) {
      ...
      this.CommonUtilsService.deleteDetailByModelName(this.Modelname, this.SelectedRows[i].id).then(obj => {
        this.$log.log(obj);
        if (obj.data.status == "error") { //eslint-disable-line
          this.text = obj.data.err;
        } else {
          this.text = "删除成功";
        }
        this.hiddenbtn = false;
...
export const templatetext = {
  template: require('./templatetext.html'),
  bindings: {
    resolve: '<',
    close: '&',
    dismiss: '&'
  },
  controller: templatetextComponentController
};
