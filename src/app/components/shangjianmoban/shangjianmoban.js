...
class shangjianmobanController extends baseGridComponent {
  // @ngInject
  constructor(GridOptionService, CommonUtilsService, $log, $scope, $state, $http) {
    ...
    this.searchCondition = {condition: {TempleteName: ''}, pageIndex: 1, sortby: '', pageSize: 25};
    this.$scope = $scope;
    this.search();
    this.$state = $state;
    this.$http = $http;
  ...
  initgridCellTemplate(columelist) {
    columelist[0].cellTemplate = '<ui><a  ui-sref="shangjianmoban.detail({id:row.entity.id})">{{row.entity.TempleteName}} </a></ui>';
  }
  saveas() {
...
export const shangjianmobancomponent = {
  template: require('./shangjianmoban.html'),
  controller: shangjianmobanController
};
